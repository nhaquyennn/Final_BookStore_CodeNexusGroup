            ...
            <?php if ($khuyenMai): ?>
              ...
              <form method="POST" action="">
                ...
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control" id="TenKhuyenMai" name="TenKhuyenMai" value="<?= $khuyenMai['TenKhuyenMai'] ?>" required>
                </div>
                <div class="mb-3">
                  <label for="PhanTramGiamGia" class="form-label">Phần trăm giảm giá (%)</label>
                  <input type="number" class="form-control" id="PhanTramGiamGia" name="PhanTramGiamGia" value="<?= $khuyenMai['PhanTramGiamGia'] ?>" required>
                </div>
                <div class="mb-3">
                  <label for="NgayBatDau" class="form-label">Ngày bắt đầu</label>
                  <input type="date" class="form-control" id="NgayBatDau" name="NgayBatDau" value="<?= $khuyenMai['NgayBatDau'] ?>" required>
                </div>
                <div class="mb-3">
                  <label for="NgayKetThuc" class="form-label">Ngày kết thúc</label>
                  <input type="date" class="form-control" id="NgayKetThuc" name="NgayKetThuc" value="<?= $khuyenMai['NgayKetThuc'] ?>" required>
                </div>
                <div class="mb-3">
                  <label for="maCTPM" class="form-label">Chọn Mã CTPM</label>
                  <select class="form-control" id="maCTPM" name="maCTPM" required>
                    <option value="" disabled>-- Chọn mã CTPM --</option>
                    <?php
                    $ctpmList = $controller->getAllCTPMFromKhuyenMai();
                    foreach ($ctpmList as $ctpm): ?>
                      <option value="<?= $ctpm['maCTPM'] ?>" <?= $khuyenMai['maCTPM'] == $ctpm['maCTPM'] ? 'selected' : '' ?>>
                        <?= $ctpm['maCTPM'] ?>
                      </option>
                    <?php endforeach; ?>
                  </select>
