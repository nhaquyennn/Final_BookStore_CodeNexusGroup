                ...
                <div class="container">
                    ...
                    <form id="form-create-phieumuon" method="POST" action="">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="d-flex">
                                <input type="text" id="search_anpham" class="form-control"
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sqlPhieuMuon = "INSERT INTO phieumuon (SoDienThoai, TongTien, hoTen, tinhTrang) VALUES (?, ?, ?, 'Đang xử lý')";
    $stmtPhieuMuon = mysqli_prepare($conn, $sqlPhieuMuon);
    mysqli_stmt_bind_param($stmtPhieuMuon, 'sss', $SDT, $TongTien, $tenKH);  // Chỉ cần 3 tham số, không cần tham số cho 'Đang xử lý'
    ...
    $sqlChiTietPhieuMuon = "INSERT INTO chitietpm (MaPhieuMuon, maAnPham, SoLuong, DonGia, ngayMuon, NgayTra) VALUES (?, ?, ?, ?, ?, ?)";
    $stmtChiTietPhieuMuon = mysqli_prepare($conn, $sqlChiTietPhieuMuon);

    // Duyệt qua các mục đã chọn
    foreach ($selectedItems as $item) {
        // Sử dụng kiểu dữ liệu phù hợp cho từng tham số
        mysqli_stmt_bind_param($stmtChiTietPhieuMuon, 'iisids', $maPhieuMuon, $item['maAnPham'], $item['soLuong'], $item['PhiThue'], $NgayMuon, $NgayTra);
